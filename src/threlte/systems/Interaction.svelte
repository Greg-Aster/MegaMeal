...
onMount(async () => {
  ...
  try {
    // Create interaction system with EventBus-like object
    const eventBus = {
      emit: (event: string, data: any) => {
        dispatch(event, data)
        handleInteractionEvent(event, data)
      },
      on: () => {}, // Not used in this context
      off: () => {} // Not used in this context
    }
    
    interactionSystem = new InteractionSystem(camera, container, eventBus)
...
function handleMouseMove(event: MouseEvent) {
  if (!enableRaycasting || !interactionSystem) return
...
useTask((delta) => {
  if (interactionSystem && isInitialized) {
    // Update with camera position for distance-based interactions
    if (camera) {
      interactionSystem.update(delta, camera.position)
    }
  }
})
...
// Export interaction system for external access
