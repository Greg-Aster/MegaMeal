...
export class StarTextureAtlas {
   ...
   */
  private generateStarFrame(ctx: CanvasRenderingContext2D, offsetX: number, offsetY: number, frameIndex: number): void {
    ...
    if (twinkleIntensity > 0.5) {
      this.drawStarSpikes(ctx, centerX, centerY, coreRadius * 1.5, brightness * twinkleIntensity);
    }
  }
   ...
   */
  private drawStarSpikes(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, length: number, opacity: number): void {
   ...
   */
  public getFrameForTime(time: number, animationSpeed: number = 1.0): number {
