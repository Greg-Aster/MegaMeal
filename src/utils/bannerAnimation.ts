...
import type { BannerAnimationConfig, BannerItemPreviewDetails } from '@/config/banners/types'
...
import { BannerPreviewCards, type PreviewCardConfig } from './BannerPreviewCards'
import { BannerVideoManager, VideoLoadingState, type VideoManagerConfig } from './BannerVideoManager'
...
export class BannerAnimationController {
  ...
  private manualNavigationDelay: number = 5000;
  ...
  constructor(config: BannerControllerConfig) {
    ...
    this.videoManager = new BannerVideoManager({
      onLoadingStateChange: (index, state) => this.handleVideoStateChange(index, state),
  ...
  private updateInteractionStates(): void {
    ...
    this.bannerSlides.forEach((slide, index) => {
      const slideElement = slide as HTMLElement;
      slideElement.classList.toggle('banner-interactive', index === this.currentIndex);
      slideElement.classList.toggle('banner-non-interactive', index !== this.currentIndex);
    });
  }
  ...
  public navigateToPrevious(): void {
    ...
    const prevIndex = this.currentIndex > 0 ? this.currentIndex - 1 : this.bannerSlides.length - 1;
  ...
  private setupKeyboardNavigation(): void {
    document.addEventListener('keydown', (e) => {
      ...
      switch (e.key) {
        ...
        case 'End':
          e.preventDefault();
          this.navigateToIndex(this.bannerSlides ? this.bannerSlides.length - 1 : 0);
          break;
      }
    });
  }
  ...
  private setupNavigationEvents(): void {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (positionDot) {
        e.preventDefault();
        e.stopPropagation();
        const index = parseInt(positionDot.getAttribute('data-index') || '0');
  ...
  private handleVideoStateChange(index: number, state: VideoLoadingState): void {
    // Handle loading indicators through the video manager
    if (state === VideoLoadingState.LOADING) {
      this.showLoadingIndicator(index);
    } else if (state === VideoLoadingState.LOADED || state === VideoLoadingState.ERROR) {
  ...
  private showLoadingIndicator(index: number): void {
    ...
    const indicator = slide.querySelector('.banner-loading-indicator') as HTMLElement;
  ...
  public isPausedState(): boolean { return this.isPaused; }
  public getTotalSlides(): number { return this.bannerSlides?.length || 0; }
  public isManuallyNavigating(): boolean { return this.isManualNavigation; }

  // Compatibility methods for existing code
  public startAnimation(): void { this.start(); }
  public stopAnimation(): void { this.stop(); }
  public next(): void { this.animateToNextSlide(); }

  // Delegated methods
  public isVideoSlide(index: number): boolean { return this.videoManager.isVideoSlide(index); }
  public getVideoLoadingState(index: number): VideoLoadingState { return this.videoManager.getVideoLoadingState(index); }
  public showPreview(index: number): void { this.previewCards.showPreviewCard(index); }
  public hidePreview(index: number): void { /* Handled by preview cards module */ }
  public hideAllPreviews(): void { this.previewCards.hideAllPreviewCards(); }
...
export function createBannerAnimation(config: BannerControllerConfig): BannerAnimationController {
