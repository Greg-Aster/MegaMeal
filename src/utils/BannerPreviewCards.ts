...
export class BannerPreviewCards {
  ...
  private activePreviewIndex: number = -1; // Track which preview is currently active
   ...
   */
  private createPreviewCardForSlide(slideElement: HTMLElement, index: number): void {
    ...
    const clickIndicator = this.createClickIndicator(previewDetails.hasValidLink);
   ...
   */
  private createPreviewOverlay(previewDetails: BannerItemPreviewDetails, index: number): HTMLElement {
    const {
      hasValidLink,
      originalHref,
      urlForDisplay,
   ...
   */
  private getPreviewCardTemplate(data: BannerItemPreviewDetails & { index: number }): string {
    const {
      hasValidLink,
      originalHref,
      urlForDisplay,
  ...
  private getPreviewContentTemplate(
    ...
    return `
      ...
      ${urlForDisplay ? `<div class="text-black/30 dark:text-white/30 text-sm transition mb-4 flex items-center">
  ...
  private setupGlobalClickHandler(): void {
    document.addEventListener('click', (e) => {
      ...
      const activeSlide = this.bannerSlides?.[this.activePreviewIndex] as HTMLElement;
      if (!activeSlide) return;
      
      const activePreview = activeSlide.querySelector('.banner-postcard-preview') as HTMLElement;
      const bannerLink = activeSlide.querySelector('.banner-link') as HTMLElement;
      ...
      const isClickOnToggler = bannerLink && bannerLink.contains(clickedElement);
      
      // Check if click is on navigation buttons (these should not close the preview)
      const isNavigationClick = clickedElement.closest('.banner-nav-prev, .banner-nav-next, .banner-position-dot');
   ...
   */
  private setupPreviewCardEvents(bannerLinkElement: HTMLElement, previewOverlay: HTMLElement, index: number): void {
  ...
  public showPreviewCard(index: number): void {
    if (!this.bannerSlides || index < 0 || index >= this.bannerSlides.length) return;

    const slide = this.bannerSlides[index] as HTMLElement;
    const previewOverlay = slide.querySelector('.banner-postcard-preview') as HTMLElement;
  ...
  public hideAllPreviewCards(): void {
    ...
    this.bannerSlides.forEach((slide, index) => {
      ...
      if (previewOverlay && !previewOverlay.classList.contains('banner-preview-hidden')) {
        this.hidePreviewCard(previewOverlay, index);
      }
    });
