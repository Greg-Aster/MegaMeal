...
export function hashCode(str: string): number {
  if (!str) return 0;
  return Math.abs(str.split('').reduce((a, b) => ((a << 5) - a) + b.charCodeAt(0), 0));
}

/**
 * Get star color based on era or hash
 */
export function getStarColor(id: string, era?: string, useEraColors?: boolean): string {
...
export const CONSTELLATION_CONFIG = {
  ...
  'conflict-epoch': { center: [180, 40], spread: 40, pattern: 'crossed_swords' },
  'singularity-conflict': { center: [240, 45], spread: 35, pattern: 'supernova' },
...
export function createSimpleStarTexture(
  ...
  // Outer glow
  const gradient = ctx.createRadialGradient(center, center, 0, center, center, radius * 3);
...
export const deviceUtils = {
  ...
  isMobilePortrait: () => deviceUtils.isMobile() && window.matchMedia("(orientation: portrait)").matches,
...
export function positionCard(
  ...
  if (!screenPosition.isInFront) {
    cardElement.className = 'fixed opacity-0 pointer-events-none transition-opacity duration-300 z-[200]';
    return;
  }
  ...
  cardElement.className = 'fixed opacity-100 pointer-events-auto transition-opacity duration-300 z-[200]';
}

/**
 * Three.js script loader with promises
 */
