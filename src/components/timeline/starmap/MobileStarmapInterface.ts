// src/components/timeline/starmap/MobileStarmapInterface.ts
...
export class MobileStarmapInterface {
  private readonly starmapContainerId: string;
  private readonly options: Required<MobileCommandOptions>;
  
  // State tracking
  private isAutoRotating = false;
  private currentFOV = 60;
  private readonly minFOV = 15;
  private readonly maxFOV = 75;
  
  // Callbacks for UI updates
  private onAutoRotateChange: ((isAutoRotating: boolean) => void) | null = null;
  private onFOVChange: ((fov: number, zoomLevel: number) => void) | null = null;
  private onViewReset: (() => void) | null = null;

  constructor(starmapContainerId: string, options: MobileCommandOptions = {}) {
  ...
  private bindMethods(): void {
    ...
    methods.forEach(method => {
      ...
      if (typeof originalMethod === 'function') {
        (this[method] as any) = originalMethod.bind(this);
      }
    });
  }

  // === STARMAP ACCESS ===
  private getStarmapAPI(): StarmapAPI | null {
    const api = (window as any)[`starmap_${this.starmapContainerId}`];
    return api || null;
  }
  ...
  // === DATA COMMANDS ===
  public updateEvents(newEvents: StarEvent[]): boolean {
    const api = this.getStarmapAPI();
  ...
  // === BATCH COMMANDS ===
  public async executeSequence(commands: MobileCommand[]): Promise<CommandResult[]> {
  ...
  // === PRESET COMMANDS ===
  public async quickTour(): Promise<CommandResult[]> {
  ...
  public async focusMode(): Promise<CommandResult[]> {
    return await this.executeSequence([
  ...
  public async overviewMode(): Promise<CommandResult[]> {
    return await this.executeSequence([
      { action: 'resetView' },
      { action: 'zoomOut' },
  ...
  // === EVENT SYSTEM ===
  private emitCommand(commandName: string, data: Record<string, any> = {}): void {
  ...
  public onAutoRotateChanged(callback: (isAutoRotating: boolean) => void): void {
    this.onAutoRotateChange = callback;
  }

  public onFOVChanged(callback: (fov: number, zoomLevel: number) => void): void {
    this.onFOVChange = callback;
  }

  public onViewResetted(callback: () => void): void {
