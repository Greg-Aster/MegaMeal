...
export function setupBleepy(mascotContextPropValue?: string, instanceId?: string) {
  ...
  function hideMascot(isDismissal = true) {
    ...
    if (isDismissal) {
      ...
      if (mascotSpeechBubble && mascotSpeechText && (mascotSpeechText as HTMLElement).textContent) {
        // Speech handling
      }
  ...
  const mainMascotContainer = document.getElementById('mascot-container') as HTMLElement; // Renamed to avoid conflict
