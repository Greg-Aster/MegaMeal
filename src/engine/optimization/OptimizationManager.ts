...
export class OptimizationManager {
  ...
  private readonly configProfiles: Record<
    OptimizationLevel,
    OptimizationConfig
  > = {
    [OptimizationLevel.ULTRA_LOW]: {
  ...
  public optimizeLevel(
    ...
    console.log(
      '🎛️ INSTANCED RENDERING: Starting automated level optimization with instanced rendering...',
    )

    console.log(
      '🎛️ OptimizationManager: Starting automated level optimization...',
    )
    ...
    console.log(
      `🎛️ Found ${vegetationObjects.length} vegetation objects to cluster`,
    )

    // STEP 2: Group objects by type for instanced rendering
    const instancedGroups = this.createInstancedGroups(vegetationObjects)

    // STEP 3: Replace individual objects with instanced meshes
    this.replaceIndividualObjectsWithInstancedMeshes(
      levelGroup,
      vegetationObjects,
      instancedGroups,
    )
    ...
    console.log(
      `✅ OptimizationManager: Created ${instancedGroups.size} InstancedMeshes from ${vegetationObjects.length} objects`,
    )
  }
  ...
  /**
   * STEP 2: Group objects by type for instanced rendering instead of spatial clustering
   */
  private createInstancedGroups(
    objects: THREE.Object3D[],
  ): Map<string, InstancedGroup> {
    const instancedGroups = new Map<string, InstancedGroup>()

    objects.forEach(object => {
      // Group by object type (geometry + material combination)
      const groupKey = this.getObjectGroupKey(object)

      if (!instancedGroups.has(groupKey)) {
        instancedGroups.set(groupKey, {
          masterObject: object,
          instances: [],
          transformMatrices: [],
        })
      ...
      const group = instancedGroups.get(groupKey)!
      group.instances.push(object)

      // Calculate and store world transformation matrix
      object.updateWorldMatrix(true, false)
      group.transformMatrices.push(object.matrixWorld.clone())
    })
  ...
  /**
   * Generate a unique key for grouping similar objects for instancing
   */
  private getObjectGroupKey(object: THREE.Object3D): string {
    if (object instanceof THREE.Mesh) {
      ...
      // Use geometry UUID and material UUID to group identical objects
      const geometryId = geometry?.uuid || 'no-geometry'
      const materialId = material?.uuid || 'no-material'

      return `${geometryId}_${materialId}`
    }

    // For non-mesh objects, group by name pattern
    const name = object.name.toLowerCase()
    ...
    return object.constructor.name + '_' + (object.name || 'unnamed')
  }

  /**
   * STEP 3: Replace individual objects with INSTANCED MESHES
   * PERFORMANCE CRITICAL: Uses GPU instancing to eliminate draw call bottlenecks
   */
  private replaceIndividualObjectsWithInstancedMeshes(
    levelGroup: THREE.Group,
    originalObjects: THREE.Object3D[],
    instancedGroups: Map<string, InstancedGroup>,
  ): void {
    ...
    // Create InstancedMesh for each object type
    instancedGroups.forEach((group, groupKey) => {
      const instancedMesh = this.createInstancedMesh(group, groupKey)
      if (instancedMesh) {
        levelGroup.add(instancedMesh)

        // Register the instanced mesh for optimization
        this.autoRegisterObject(instancedMesh)
      }
    })

    const totalInstances = Array.from(instancedGroups.values()).reduce(
      (sum, group) => sum + group.instances.length,
      0,
    )

    console.log(
      `🚀 GPU Instancing: Converted ${originalObjects.length} objects into ${instancedGroups.size} InstancedMeshes with ${totalInstances} instances`,
    )
  }

  /**
   * INSTANCED RENDERING: Convert a group of similar objects into a single InstancedMesh
   * This is the key optimization that eliminates GPU draw call bottlenecks using GPU instancing
   */
  private createInstancedMesh(
    ...
    try {
      const masterObject = group.masterObject

      if (
        !(masterObject instanceof THREE.Mesh) ||
        !masterObject.geometry ||
        !masterObject.material
      ) {
        console.warn(
          `Cannot create InstancedMesh for non-mesh object: ${groupKey}`,
        )
        return null
      }

      const instanceCount = group.instances.length
      const geometry = masterObject.geometry.clone()
      const material = Array.isArray(masterObject.material)
        ? masterObject.material[0]
        : masterObject.material

      // Create InstancedMesh with the master geometry and material
      const instancedMesh = new THREE.InstancedMesh(
        geometry,
        material,
        instanceCount,
      )
      instancedMesh.name = `instanced_${groupKey}_${instanceCount}instances`
      instancedMesh.castShadow = true
      instancedMesh.receiveShadow = true

      // Disable default frustum culling - let OptimizationManager handle culling
      instancedMesh.frustumCulled = false

      // Apply transformation matrices to each instance
      group.transformMatrices.forEach((matrix, index) => {
      ...
      console.log(
        `🔧 Created InstancedMesh: ${instancedMesh.name} with ${instanceCount} instances`,
      )
      return instancedMesh
    } catch (error) {
      console.error(
        `Failed to create InstancedMesh for group ${groupKey}:`,
        error,
      )
      return null
    }
  ...
  private manualMergeGeometries(
    ...
    geometries.forEach(geometry => {
      ...
      if (indices) {
        const offsetIndices = Array.from(indices.array).map(
          i => i + currentIndexOffset,
        )
  ...
  private detectDeviceCapabilities(): void {
    ...
    if (
      ...
      console.warn(
        '⚠️ OptimizationManager: Cannot detect device capabilities in a non-browser environment. Using defaults.',
      )
      return
    }
  ...
  public getQualitySettings(): QualitySettings {
    return (
      ...
      this.qualityProfiles[this.currentOptimizationLevel]
    )
  }
  ...
  public optimizeTexture(texture: THREE.Texture): THREE.Texture {
    ...
    if (
      ...
      this.currentOptimizationLevel === OptimizationLevel.ULTRA_LOW ||
      this.currentOptimizationLevel === OptimizationLevel.LOW
    ) {
  ...
  public optimizeMaterial(material: THREE.Material): THREE.Material {
    ...
    if (
      ...
      if (this.deviceCapabilities.isLowEnd && stdMaterial.roughnessMap) {
        stdMaterial.roughnessMap = this.optimizeTexture(
      ...
      if (stdMaterial.normalMap) {
        ...
        if (this.deviceCapabilities.isLowEnd && stdMaterial.normalScale) {
          stdMaterial.normalScale.setScalar(
            Math.min(stdMaterial.normalScale.x * 0.7, 1.0),
          )
        }
      }
   ...
   */
  public createLODMesh(originalMesh: THREE.Mesh, lodLevel = 1): THREE.Mesh {
    if (!this.deviceCapabilities?.isMobile || lodLevel === 0)
      return originalMesh
    ...
    if (geometry instanceof THREE.BufferGeometry) {
      ...
      if (positions && indices) {
        ...
        const newIndexCount = Math.max(
          6,
          Math.floor(indices.count / decimationFactor),
  ...
  public disableOptimization(): void {
    ...
    this.managedObjects.forEach(managedObject => {
      ...
      if (
        managedObject.object instanceof THREE.Mesh &&
        managedObject.object.material
      ) {
    ...
    this.scene.traverse(object => {
      if (
        ...
        const shouldBeActive =
          isInFrustum && distance <= this.config.maxRenderDistance * 1.5
  ...
  private shouldOptimizeObject(object: THREE.Object3D): boolean {
    ...
    if (
      ...
      name.match(/(grass|flower|particle)/i) &&
      !name.match(/fireflies/i)
    ) {
      return true
    }
  ...
  private isEssentialObject(object: THREE.Object3D): boolean {
    ...
    if (
      name.match(/(ground|terrain|floor|base|platform|skybox|sky|water|pool)/i)
    ) {
      return true
    }
    ...
    if (
      ...
      name.match(/(camera|control|helper)/i)
    ) {
      return true
    }
  ...
  private isDecorationMesh(object: THREE.Mesh): boolean {
    ...
    if (
      name.match(/(ground|terrain|floor|water|main|base|primary|skybox|sky)/i)
    ) {
      return false
    }
    ...
    if (
      name.match(
        /(prop|decoration|ornament|detail|small|rock|stone|tree|bush|flower|grass)/i,
      )
    ) {
      return true
    }
  ...
  private autoRegisterObject(object: THREE.Object3D): void {
    ...
    if (
      ...
      console.warn(
        `🚨 Attempted to register lighting object for optimization: "${object.name}" - BLOCKED`,
      )
      return
    }
  ...
  ): 'vegetation' | 'decoration' | 'structure' | 'effect' {
    const name = object.name.toLowerCase()
  ...
  public update(deltaTime: number): void {
    ...
    if (batchSize > 0 && this.lastOptimizationCheck % 100 < 50) {
      // Only 50% of frames
      for (let i = 0; i < batchSize; i++) {
        // Cycle through objects to ensure all get updated over time
        const objectIndex =
          (Math.floor(this.lastOptimizationCheck / 10) + i) %
  ...
  private optimizeObject(object: THREE.Object3D, deltaTime: number): void {
    ...
    const managedObject = Array.from(this.managedObjects.values()).find(
      mo => mo.object === object,
    )
    ...
    object.traverse(child => {
      ...
      if (child instanceof THREE.Light) {
        if (child.castShadow && distance > this.config.shadowDistance) {
          ...
          !child.castShadow &&
          distance <= this.config.shadowDistance
        ) {
          ...
          if (
            ...
            if (child.shadow) {
              const optimalSize = Math.min(
                ...
                distance < this.config.shadowDistance * 0.5
                  ? this.config.maxShadowMapSize
  ...
  private applyOptimization(
    ...
    if (distance > fadeStartDistance && distance <= fadeEndDistance) {
      ...
      const fadeProgress =
        (distance - fadeStartDistance) / this.config.fadeDistance
  ...
  private applySmoothFade(
    ...
    if (this.deviceCapabilities?.isMobile && targetOpacity < 0.5) {
      console.log(
        `🌫️ Fading object: "${managedObject.object.name}" to opacity ${targetOpacity.toFixed(2)}`,
      )
    }

    // Smoothly interpolate to target opacity
    const opacityDelta =
      (targetOpacity - managedObject.currentOpacity) *
      this.config.fadeSpeed *
      deltaTime
    managedObject.currentOpacity += opacityDelta

    // Clamp opacity
    managedObject.currentOpacity = Math.max(
      0,
      Math.min(1, managedObject.currentOpacity),
    )
    ...
    this.applyOpacityToObject(
      managedObject.object,
      managedObject.currentOpacity,
    )
  ...
  private applyOpacityToObject(object: THREE.Object3D, opacity: number): void {
      ...
      object.traverse(child => {
        if (child instanceof THREE.Mesh) {
          ...
          materials.forEach(material => {
            if (
              material &&
              !object.userData.cachedMaterials.includes(material)
            ) {
    ...
    object.userData.cachedMaterials.forEach((material: any) => {
      if (material) {
        ...
        if (
          material.userData.optimizationSystemOwner &&
  ...
  private reportOptimizationStats(): void {
    ...
    if (Date.now() % 30000 < this.config.checkInterval) {
      const stats = {
        ...this.optimizationStats,
        managedObjectsCount: this.managedObjects.size,
        visibleObjectsCount: Array.from(this.managedObjects.values()).filter(
          mo => mo.isVisible,
        ).length,
        loadedObjectsCount: Array.from(this.managedObjects.values()).filter(
          mo => mo.isLoaded,
        ).length,
      }
...
/**
 * Interface for grouping objects for instanced rendering
 */
interface InstancedGroup {
  masterObject: THREE.Object3D
  instances: THREE.Object3D[]
  transformMatrices: THREE.Matrix4[]
}
