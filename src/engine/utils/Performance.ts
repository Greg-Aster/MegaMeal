...
export class Performance {
  ...
  // Memory monitoring
  private memoryInfo: { used: number; total: number; limit: number } | null =
    null
  ...
  private checkMobilePerformance(): void {
    ...
    if (this.frameCount % 300 === 0) {
      ...
      if (metrics.fps < 20) {
        console.warn(
          `🐌 Poor mobile performance: ${metrics.fps.toFixed(1)} FPS`,
        )
        console.warn(
          `📊 Frame time: ${metrics.frameTime.toFixed(1)}ms (target: 16.7ms)`,
        )
        ...
        console.warn(
          `🎨 Render stats: ${metrics.renderCalls} calls, ${metrics.triangles} triangles`,
        )
      }
    }
  }
  ...
  public analyzePerformance(): {
    ...
    if (this.fpsHistory.length > 0) {
      analysis.averageFPS = Math.round(
