...
export class Debug {
  ...
  private setupGui(): void {
    this.gui = new GUI({
      width: this.config.guiWidth,
      closed: this.config.guiClosed,
    })
  ...
  private setupEngineFolder(): void {
    ...
    folder
      .add(this.debugData.engine, 'deltaTime', 0, 0.1)
      .name('Delta Time')
      .listen()
      .disable()
    folder
      .add(this.debugData.engine, 'totalTime')
      .name('Total Time')
      .listen()
      .disable()
    folder
      .add(this.debugData.engine, 'isRunning')
      .name('Running')
      .listen()
      .disable()

    folder
      .add({ pause: () => this.engine.pause() }, 'pause')
      .name('Pause Engine')
    folder
      .add({ resume: () => this.engine.resume() }, 'resume')
      .name('Resume Engine')
  ...
  private setupRendererFolder(): void {
    ...
    folder
      .add(this.debugData.renderer, 'drawCalls')
      .name('Draw Calls')
      .listen()
      .disable()
    folder
      .add(this.debugData.renderer, 'triangles')
      .name('Triangles')
      .listen()
      .disable()
    folder
      .add(this.debugData.renderer, 'geometries')
      .name('Geometries')
      .listen()
      .disable()
    folder
      .add(this.debugData.renderer, 'textures')
      .name('Textures')
      .listen()
      .disable()
    ...
    // Pixel ratio control
    folder
      .add({ pixelRatio: window.devicePixelRatio }, 'pixelRatio', 0.5, 3, 0.1)
      .name('Pixel Ratio')
  ...
  private setupPhysicsFolder(): void {
    ...
    folder
      .add(this.debugData.physics.gravity, 'x', -20, 20, 0.1)
      .name('Gravity X')
      .onChange(() => this.updateGravity())

    folder
      .add(this.debugData.physics.gravity, 'y', -20, 20, 0.1)
      .name('Gravity Y')
      .onChange(() => this.updateGravity())

    folder
      .add(this.debugData.physics.gravity, 'z', -20, 20, 0.1)
      .name('Gravity Z')
      .onChange(() => this.updateGravity())

    folder
      .add({ debugRender: false }, 'debugRender')
      .name('Debug Render')
  ...
  private setupInputFolder(): void {
    ...
    folder
      .add(this.debugData.input, 'mouseSensitivity', 0.0001, 0.01, 0.0001)
      .name('Mouse Sensitivity')
      .onChange((value: number) => inputManager.setMouseSensitivity(value))

    folder
      .add(this.debugData.input, 'touchSensitivity', 0.001, 0.02, 0.001)
      .name('Touch Sensitivity')
      .onChange((value: number) => inputManager.setTouchSensitivity(value))

    folder
      .add(this.debugData.input, 'pointerLocked')
      .name('Pointer Locked')
      .listen()
      .disable()
    ...
    folder
      .add(
        { requestLock: () => inputManager.requestPointerLock() },
        'requestLock',
      )
      .name('Request Pointer Lock')

    folder
      .add({ exitLock: () => inputManager.exitPointerLock() }, 'exitLock')
      .name('Exit Pointer Lock')

    folder
      .add({ invertY: false }, 'invertY')
      .name('Invert Y')
  ...
  private setupPostProcessingFolder(): void {
    ...
    folder
      .add(this.debugData.postProcessing, 'bloomIntensity', 0, 2, 0.1)
      .name('Bloom Intensity')
      .onChange((value: number) => renderer.setBloomIntensity(value))

    folder
      .add(this.debugData.postProcessing, 'ssaoIntensity', 0, 2, 0.1)
      .name('SSAO Intensity')
      .onChange((value: number) => renderer.setSSAOIntensity(value))

    folder
      .add(this.debugData.postProcessing, 'toneMappingExposure', 0, 3, 0.1)
      .name('Exposure')
  ...
  private setupUtilityFolder(): void {
    ...
    folder
      .add({ clearConsole: () => console.clear() }, 'clearConsole')
      .name('Clear Console')
    folder
      .add({ logDebugInfo: () => this.logDebugInfo() }, 'logDebugInfo')
      .name('Log Debug Info')
    folder
      .add({ toggleStats: () => this.toggleStats() }, 'toggleStats')
      .name('Toggle Stats')
  }
