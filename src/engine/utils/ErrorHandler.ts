...
export class ErrorHandler {
   ...
   * Get recent errors
   */
  public static getRecentErrors(count: number = 10): ErrorEntry[] {
  ...
  public static validateInitialization<T>(
    ...
    if (result === null || result === undefined) {
      throw new Error(`Initialization failed: ${context.operation} returned null/undefined`);
    }
    return result;
  }
   ...
   */
  public static assert(condition: boolean, message: string, context: ErrorContext): void {
    if (!condition) {
  ...
  private static generateRecoverySuggestions(error: Error, context: ErrorContext): string[] {
...
export const ErrorUtils = {
   ...
   */
  getSeverity(error: Error, context: ErrorContext): 'low' | 'medium' | 'high' | 'critical' {
    if (error.message.includes('out of memory') || error.message.includes('WebGL context lost')) {
      return 'critical';
    }
