...
export class AssetLoader {
  ...
  private loadFBXModel(id: string, fbxPath: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.fbxLoader.load(
        ...
        error => {
          console.error(
            `❌ Failed to load FBX model ${id} from ${fbxPath}:`,
            error,
          )
          reject(error)
        },
   ...
   */
  public async loadPBRTextureSet(
    id: string,
    paths: PBRTextureSetPaths,
    ...
    if (paths.map) {
      promises.push(
        this.loadTexture(`${id}_map`, paths.map).then(t => {
          textureSet.map = t
        }),
      )
    }
    if (paths.normalMap) {
      promises.push(
        this.loadTexture(`${id}_normal`, paths.normalMap).then(t => {
          textureSet.normalMap = t
        }),
      )
    }
    if (paths.roughnessMap) {
      promises.push(
        this.loadTexture(`${id}_roughness`, paths.roughnessMap).then(t => {
          textureSet.roughnessMap = t
        }),
      )
    }
    ...
    if (paths.aoMap) {
      promises.push(
        this.loadTexture(`${id}_ao`, paths.aoMap).then(t => {
          textureSet.aoMap = t
        }),
      )
    }
    if (paths.metalnessMap) {
      promises.push(
        this.loadTexture(`${id}_metal`, paths.metalnessMap).then(t => {
          textureSet.metalnessMap = t
        }),
      )
    }
    if (paths.displacementMap) {
      promises.push(
        this.loadTexture(`${id}_disp`, paths.displacementMap).then(t => {
          textureSet.displacementMap = t
        }),
      )
    }
  ...
  public async loadManifest(manifest: AssetManifest): Promise<void> {
    ...
    if (manifest.pbrTextureSets) {
      Object.entries(manifest.pbrTextureSets).forEach(([id, paths]) =>
        promises.push(this.loadPBRTextureSet(id, paths)),
      )
    }
    if (manifest.models) {
  ...
  public preloadTextures(textureUrls: { [key: string]: string }): Promise<
    THREE.Texture[]
  > {
