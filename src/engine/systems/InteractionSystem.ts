...
export class InteractionSystem {
  ...
  private handleMouseUp(event: MouseEvent): void {
    ...
    const mouseDownTime =
      event.timeStamp - ((window as any).lastMouseDownTime || 0)
    if (mouseDownTime < 200) {
      // Less than 200ms = click
  ...
  private handleTouchEnd(event: TouchEvent): void {
    ...
    const touchDownTime =
      event.timeStamp - ((window as any).lastTouchDownTime || 0)

    if (touchDownTime < 200) {
      // Quick tap
  ...
  private handleClick(
    ...
    if (result) {
      ...
      const interactionData: InteractionData = {
        ...
        direction: result.worldPosition
          .clone()
  ...
  private performRaycast(
    ...
    const meshToInteractable: Map<THREE.Object3D, InteractableObject> =
      new Map()
  ...
  private checkProximityInteractions(playerPosition: THREE.Vector3): void {
    ...
    this.interactables.forEach(interactable => {
      ...
      const wasInRange =
        this.lastPlayerPosition.distanceTo(interactable.mesh.position) <=
      ...
      if (isInRange) {
        const interactionData: InteractionData = {
          ...
          cameraPosition: this.camera.position.clone(),
          direction: interactable.mesh.position
            .clone()
            .sub(this.camera.position)
            .normalize(),
  ...
  public getStats(): InteractionSystemStats {
    return {
      ...
      enabledInteractables: Array.from(this.interactables.values()).filter(
        i => i.isInteractable,
      ).length,
