...
export class FireflySystem extends GameObject {
  ...
  constructor(
    ...
    console.log(
      `✨ FireflySystem created: ${this.config.count} fireflies, ${this.config.maxLights} max lights`,
    )
  }
  ...
  private createSingleFirefly(index: number): FireflyData {
    ...
    const y =
      groundHeight +
      this.config.heightRange.min +
      Math.random() *
        (this.config.heightRange.max - this.config.heightRange.min)
  ...
  private initializeLightCycling(): void {
    ...
    const initialActiveCount = Math.min(
      this.config.maxLights,
      this.activeLights.length,
    )
    ...
    for (let i = 0; i < initialActiveCount; i++) {
      ...
      const targetIndex = Math.floor(
        (i / initialActiveCount) * this.fireflies.length,
      )
      ...
      if (firefly && firefly.light) {
        ...
        firefly.lightCycleTime =
          (i / initialActiveCount) * this.config.cycleDuration * 0.8
      }
    }
  }
  ...
  private updateFireflyPositions(deltaTime: number): void {
    const { speed, wanderSpeed, wanderRadius, floatAmplitude, lerpFactor } =
    ...
    this.fireflies.forEach((firefly, index) => {
      ...
      const wanderX =
        Math.sin(this.animationTime * wanderSpeed + index) * wanderRadius
      const wanderZ =
        Math.cos(this.animationTime * wanderSpeed + index * 1.3) * wanderRadius
  ...
  private cycleMultipleLights(): void {
    ...
    for (let i = 0; i < cyclesToPerform; i++) {
      ...
      const candidatesForDeactivation = this.fireflies.filter(
        f =>
          f.light &&
          f.isLightActive &&
          f.lightCycleTime > this.config.cycleDuration * 0.3,
      )
      ...
      if (candidatesForDeactivation.length > 0) {
        ...
        const randomActive =
          candidatesForDeactivation[
            Math.floor(Math.random() * candidatesForDeactivation.length)
          ]
  ...
  private replaceOldestLight(): void {
    ...
    this.fireflies.forEach(firefly => {
      if (
        firefly.light &&
        firefly.isLightActive &&
        firefly.lightCycleTime > maxCycleTime
      ) {
  ...
  private activateLightInDifferentRegion(
    deactivatedFirefly: FireflyData,
  ): void {
    const inactiveLights = this.fireflies.filter(
      f => f.light && !f.isLightActive,
    )
    ...
    const targetFirefly =
      maxDistance > this.config.radius * 0.3
        ? bestCandidate
   ...
   */
  private updateCameraAwareLighting(
    deltaTime: number,
    camera: THREE.Camera,
  ): void {
  ...
  private updateLightFading(deltaTime: number): void {
    this.fireflies.forEach(firefly => {
      ...
      if (Math.abs(fadeDirection) > 0.001) {
        ...
        firefly.lightFadeProgress = Math.max(
          0.0,
          Math.min(1.0, firefly.lightFadeProgress),
        )
      }
