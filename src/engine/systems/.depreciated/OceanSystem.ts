...
export class OceanSystem extends GameObject {
  ...
  constructor(
    ...
    console.log(
      `🌊 OceanSystem created: ${this.config.size.width}x${this.config.size.height}, ${this.config.segments.width}x${this.config.segments.height} segments`,
    )
  }
  ...
  private applyIntelligentOptimization(): void {
    try {
        ...
        qualitySettings: qualitySettings
          ? {
              ...
              enableProceduralTextures:
                qualitySettings.enableProceduralTextures,
      ...
      if (qualitySettings) {
        ...
        console.log(
          `✅ OceanSystem: Applied intelligent optimization for ${optimizationLevel}`,
          {
        ...
        console.warn(
          `🌊 OceanSystem: No quality settings available for level ${optimizationLevel}`,
        )
      }
    } catch (error) {
      console.error(
        '🌊 OceanSystem: Failed to apply intelligent optimization:',
        error,
      )
  ...
  private createOcean(): void {
    ...
    if (qualitySettings?.enableProceduralTextures) {
      ...
      console.log(
        '🌊 Ocean: Skipping procedural textures (quality settings disabled)',
      )
    }
  ...
  private getQualitySettings() {
    try {
      ...
      const qualitySettings = optimizationManager.getQualitySettings()
      console.log(
        '🌊 OceanSystem: Retrieved quality settings:',
        qualitySettings,
      )
  ...
  private createIntelligentWaterMaterial(): THREE.Material {
    ...
    const optimizationLevel =
      optimizationManager?.getOptimizationLevel() || 'medium'

    console.log(
      `🌊 Water Material: Creating for quality level ${optimizationLevel}`,
      {
        qualitySettings: qualitySettings
          ? {
              enableProceduralTextures:
    ...
    if (!qualitySettings) {
      ...
      console.log(
        '🌊 Water Material: Using fallback basic material (no quality settings)',
      )
    ...
    if (
      !qualitySettings.enableProceduralTextures &&
      !qualitySettings.enableNormalMaps
    ) {
      console.log(
        '🌊 Water Material: Using ULTRA_LOW basic material (no reflections/refractions)',
      )
      return new this.THREE.MeshBasicMaterial({
        color: 0x006994,
        transparent: true,
    ...
    if (
      !qualitySettings.enableNormalMaps ||
      (!qualitySettings.enableReflections && !qualitySettings.enableRefractions)
    ) {
      console.log(
        '🌊 Water Material: Using LOW Lambert material (no reflections/refractions)',
      )
    ...
    console.log(
      `🌊 Water Material: Using ${optimizationLevel.toUpperCase()} PBR material`,
      {
        reflections: qualitySettings.enableReflections,
        refractions: qualitySettings.enableRefractions,
      },
    ...
    if (qualitySettings.enableRefractions) {
      console.log(
        '🌊 Water Material: Enabling refractions (ultra quality only)',
      )
  ...
  private applyTexturesToMaterial(
    ...
    console.log(
      ...
      material.type,
      textureData ? 'with textures' : 'no textures',
    )
    ...
    if (
      material instanceof this.THREE.MeshStandardMaterial &&
      qualitySettings.enableNormalMaps &&
  ...
  private createProceduralTextures() {
    ...
    const waveNoise = (x: number, y: number, time = 0) => {
      let value = 0
      // Large ocean swells
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('optimizationLevelChanged', (event: Event) => {
        ...
        if (level.includes('mobile')) {
          ...
          console.log(
            '🌊 Ocean vertex animation disabled for mobile performance',
          )
        } else {
  ...
  public update(deltaTime: number): void {
    if (!this.isInitialized || !this.oceanData || !this.config.enableAnimation)
      return
  ...
  private updateWaves(time: number): void {
    ...
    if (this.shouldUpdateWaves) {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        waveDisplacement +=
          Math.cos(x * 0.0008 - y * 0.001 + time * 0.06) * 0.08 // 5x slower, smaller amplitude
  ...
  public getStats() {
    return {
      triangles: this.oceanData
        ? this.config.segments.width * this.config.segments.height * 2
        : 0,
      vertices: this.oceanData
        ? (this.config.segments.width + 1) * (this.config.segments.height + 1)
        : 0,
