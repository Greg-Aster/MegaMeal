...
export class PhysicsComponent {
  ...
  public async autoGeneratePhysics(levelGroup: THREE.Group): Promise<void> {
    if (!this.physicsWorld) {
      console.warn('PhysicsWorld not available, skipping auto-physics generation');
      return;
    }
    ...
    console.log(`✅ Auto-physics generation completed for level: ${this.levelId}`);
  }
  ...
  private async createFloorPhysics(floors: THREE.Mesh[]): Promise<void> {
    for (const floor of floors) {
      ...
      const physicsId = this.physicsWorld!.createRigidBody(floorId, physicsData, floor);
      if (physicsId) {
  ...
  private async createWallPhysics(walls: THREE.Mesh[]): Promise<void> {
    for (const wall of walls) {
      ...
      const physicsId = this.physicsWorld!.createRigidBody(wallId, physicsData, wall);
      if (physicsId) {
  ...
  private async createPropPhysics(props: THREE.Mesh[]): Promise<void> {
    for (const prop of props) {
      ...
      } else if (aspectRatio < 1.2 && Math.abs(size.x - size.y) < 0.2 && Math.abs(size.y - size.z) < 0.2) {
      ...
      const physicsId = this.physicsWorld!.createRigidBody(propId, physicsData, prop);
      if (physicsId) {
  ...
  public addPhysicsToMesh(
    ...
    const physicsId = this.physicsWorld.createRigidBody(meshId, physicsData, mesh);
    if (physicsId) {
   ...
   */
  public isPositionValid(position: THREE.Vector3, levelGroup: THREE.Group): boolean {
