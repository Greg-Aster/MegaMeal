...
// Import type guards
import { isVideoBannerItem, isImageBannerItem } from './types'
...
export const iconSVGs: Record<string, string> = {
  'book-open': '<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>',
  'user-group': '<path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>',
  'rocket': '<path d="M9.5 3A6.5 6.5 0 0116 9.5c0 1.61-.59 3.09-1.56 4.23l.27.27h.79l5 5-1.5 1.5-5-5v-.79l-.27-.27A6.516 6.516 0 019.5 16 6.5 6.5 0 013 9.5 6.5 6.5 0 019.5 3z"/>',
  ...
  'briefcase': '<path d="M16 20H8a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2z"/><path d="M12 6V4a2 2 0 00-2-2h-2a2 2 0 00-2 2v2"/>',
  'arrow-up-right-from-square': '<path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6v6M10 14L21 3"/>'
};
...
export function validateStandardBannerConfig(): {
  ...
  const itemsWithoutAlt = bannerList.filter(item => !item.alt || item.alt.trim() === '');
  ...
  const invalidVideoPaths = videoItems.filter(item => !item.src.startsWith('/videos/'));
