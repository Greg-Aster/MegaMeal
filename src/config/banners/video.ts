...
export function validateVideoBannerConfig(config: VideoBannerData): {
  ...
  if (!config.videoId || config.videoId.trim() === '') {
    ...
    errors.push('Default placeholder video ID detected. Please set a real YouTube video ID.');
  } else {
    ...
    if (!cleanVideoId) {
      ...
      warnings.push(`Video ID appears to be a URL. Consider using just the ID: ${cleanVideoId}`);
    }
  }
...
export function isVideoBannerData(data: any): data is VideoBannerData {
  return data && 'videoId' in data && typeof data.videoId === 'string' && data.videoId.trim() !== '';
}
