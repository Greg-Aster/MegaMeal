...
export function getContextualGreeting(context?: {
  ...
  const greetings = assistantGreetings[greetingCategory] || assistantGreetings.default;
...
export function getContextualQuickActions(pageType?: string) {
  ...
  const contextActions = pageType ? contextualActions[pageType as keyof typeof contextualActions] || [] : [];
...
export function validateAssistantBannerConfig(config: AssistantBannerData): {
  ...
  if (config.imageUrl) {
    if (config.imageUrl === "/path/to/assistant/background.jpg") {
      warnings.push('Default placeholder background URL detected. Consider setting a real background image.');
    } else {
      try {
        if (!config.imageUrl.startsWith('/') && !config.imageUrl.startsWith('http')) {
  ...
  if (assistantPersonality.avatar === "/assets/assistant/bleepy.png") {
    warnings.push('Default assistant avatar path detected. Consider customizing the avatar.');
  }
  
  // Check if required features are properly configured
  if (assistantBannerInteraction.autoGreet && assistantGreetings.default.length === 0) {
