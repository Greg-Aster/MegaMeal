...
export function getNoneBannerStyles(options?: {
  ...
  if (options?.consistentSpacing || noneBannerLayout.consistentSpacing.enabled) {
    const height = isMobile
 ...
 */
export function getNoneBannerContentPosition(isMobile: boolean = false): {
  top: string;
  marginTop: string;
  paddingTop: string;
} {
...
export function validateNoneBannerConfig(config: NoneBannerData): {
  ...
  if (configKeys.length > 0) {
    warnings.push('None banner configuration should be empty. Unexpected properties found: ' + configKeys.join(', '));
  }
  
  // Validate layout configuration consistency
  if (noneBannerLayout.showPlaceholder && noneBannerLayout.consistentSpacing.enabled) {
    warnings.push('Both placeholder and consistent spacing are enabled. This may cause double spacing.');
  }
  ...
  if (noneBannerLayout.showPlaceholder) {
    ...
    if (desktopHeight === '0' && mobileHeight === '0') {
      warnings.push('Placeholder is enabled but heights are set to 0. Consider disabling placeholder instead.');
    }
  }
