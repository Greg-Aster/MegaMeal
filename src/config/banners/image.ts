...
export function getResponsiveImageSources(imageUrl: string): {
  ...
  Object.entries(responsiveBreakpoints).forEach(([name, config]) => {
    ...
    if (config.maxWidth !== Infinity) {
      sizesEntries.push(`(max-width: ${config.maxWidth}px) ${config.imageWidth}px`);
    }
  });
...
export function validateImageBannerConfig(config: ImageBannerData): {
  ...
  if (!config.imageUrl || config.imageUrl.trim() === '') {
    ...
    errors.push('Default placeholder image URL detected. Please set a real image URL.');
  } else {
    // Validate URL format
    try {
    ...
    if (extension && !supportedExtensions.includes(extension)) {
      warnings.push(`Image extension ".${extension}" might not be supported. Recommended: ${supportedExtensions.join(', ')}`);
    }
  }
...
export function getImageAttributes(
  ...
  const attributes: Record<string, any> = {
    src: imageUrl,
    alt,
    loading: (customOptions?.eager || !imageDisplayConfig.lazyLoad) ? 'eager' : 'lazy',
    decoding: 'async',
 ...
 */
export function getImagePlaceholder(showGradient: boolean = true): string {
  return showGradient 
...
export function isImageBannerData(data: any): data is ImageBannerData {
  return data && 'imageUrl' in data && typeof data.imageUrl === 'string' && data.imageUrl.trim() !== '';
}
