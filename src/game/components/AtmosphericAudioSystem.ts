...
export class AtmosphericAudioSystem implements IDisposable {
    ...
    if (config.sound_effects) {
      for (const effect of config.sound_effects) {
        ...
        if (this.eventBus && typeof this.eventBus.on === 'function') {
          ...
          console.warn(
            `EventBus not available or missing 'on' method for trigger: ${effect.trigger}`,
          )
        }
      }
    }
  ...
  private playAmbientTrack(track: any): void {
    ...
    if (track.position === 'global') {
      ...
      console.log(
        `📍 Positional audio: ${track.file} at ${track.position} (radius: ${track.radius})`,
      )
    }
  }
  ...
  private playSoundEffect(effect: any): void {
    console.log(
      `🔊 Playing sound effect: ${effect.sound} at volume ${effect.volume}`,
    )
