...
export class ModelLibrary {
   ...
   */
  public async initialize(manifestPath: string = '/assets/game/shared/models/library.json'): Promise<void> {
    try {
      console.log('📚 Initializing Model Library...');
      
      const response = await fetch(manifestPath);
      if (!response.ok) {
        console.warn('⚠️ Model library manifest not found, using procedural generation only');
        return;
      }
      ...
      if (successCount > 0) {
        console.log(`✅ Model Library initialized with ${successCount}/${essentialModels.length} models`);
      } else {
        console.log('⚠️ No 3D models found, using procedural generation');
      }
    } catch (error) {
      console.warn('⚠️ Failed to initialize model library, using procedural fallback:', error);
    }
  }
   ...
   */
  public getModel(category: string, modelName: string, config: ModelConfig): THREE.Group | null {
   ...
   */
  public createForestScatter(center: THREE.Vector3, radius: number, density: number = 25): THREE.Group {
   ...
   */
  public async loadModel(category: string, modelName: string, filePath: string, format: string = 'glb', mtlPath?: string): Promise<boolean> {
    const modelId = `${category}/${modelName}`;
    ...
    try {
      console.log(`🔄 Loading ${format.toUpperCase()} model: ${modelId} from ${filePath}`);
      const model = await this.assetLoader.loadModel(modelId, filePath, format, mtlPath);
  ...
  private getEssentialModels(manifest: ModelLibraryManifest): Array<{category: string, name: string, path: string, format: string, mtl?: string}> {
    const essential = [];
  ...
  private async preloadModels(models: Array<{category: string, name: string, path: string, format: string, mtl?: string}>): Promise<boolean[]> {
    const loadPromises = models.map(model => 
      this.loadModel(model.category, model.name, model.path, model.format, model.mtl)
    );
    
    const results = await Promise.allSettled(loadPromises);
    return results.map(result => result.status === 'fulfilled' && result.value === true);
  }
  ...
  private getRandomVegetationModel(position: THREE.Vector3): THREE.Group | null {
  ...
  private getRandomDecorativeModel(position: THREE.Vector3): THREE.Group | null {
