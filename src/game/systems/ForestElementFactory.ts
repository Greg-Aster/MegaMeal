...
export class ForestElementFactory {
  ...
  public createPineTree(config: ForestElementConfig): THREE.Group {
    ...
    // Get bark texture
    const barkTextureSet = this.assetLoader.getPBRTextureSet('pine_bark') || 
    ...
    branchLayers.forEach(layer => {
      const branchGeometry = new this.THREE.ConeGeometry(layer.radius, layer.height, 8);
  ...
  public createDeadTree(config: ForestElementConfig): THREE.Mesh {
    ...
    const deadTreeSet = this.assetLoader.getPBRTextureSet('dead_tree') || 
                       this.assetLoader.getPBRTextureSet('tree_trunk');
  ...
  public populateArea(center: THREE.Vector3, radius: number, density: number = 20): THREE.Group {
