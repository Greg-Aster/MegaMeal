...
export class RoomFactory {
   ...
   */
  public async generateRoom(
    config: RoomConfig,
    level: BaseLevel,
  ): Promise<THREE.Group> {
    console.log(
      `🏗️ Generating ${config.type} room (${config.size.width}x${config.size.length}x${config.size.height})`,
    )
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `⚠️ Failed to load SciFi assets, using basic fallback:`,
          assetError,
        )
      }

      console.log(
        `✅ Generated ${config.type} room with ${fallbackRoom.children.length} elements`,
      )
      return fallbackRoom
    } catch (error) {
  ...
  private async createFloor(config: RoomConfig): Promise<THREE.Group | null> {
    ...
    try {
      ...
      console.warn(
        'Failed to load floor assets, using procedural floor:',
        error,
      )
    }
  ...
  private async createWalls(
    ...
    try {
      ...
      if (wallModel) {
        ...
        const wallPositions = [
          {
            pos: [0, config.size.height / 2, -config.size.length / 2],
            rot: [0, 0, 0],
            side: 'back',
          },
          {
          ...
          {
            pos: [-config.size.width / 2, config.size.height / 2, 0],
            rot: [0, Math.PI / 2, 0],
            side: 'left',
          },
          {
  ...
  private async createWallSegment(
    ...
    if (doorConfig && doorConfig.type !== 'blocked') {
      ...
      level.createPhysicsObject(`wall_${side}`, wall, 'static', 'cuboid', {
        friction: 0.8,
      })
    }
  ...
  private async createCeiling(config: RoomConfig): Promise<THREE.Group | null> {
    ...
    try {
      ...
      if (ceilingModel) {
        ...
        ceiling.scale.set(config.size.width / 4, 1, config.size.length / 4)
  ...
  private getPropConfigurations(config: RoomConfig): PropPlacement[] {
    ...
    switch (config.type) {
      case 'restaurant':
        props.push(
          {
            asset: 'Prop_Crate3.gltf',
            position: new this.THREE.Vector3(-6, 0.75, -6),
            physics: true,
          },
          {
            asset: 'Prop_Crate4.gltf',
            position: new this.THREE.Vector3(6, 0.75, -6),
            physics: true,
          },
          {
            asset: 'Prop_Barrel_Large.gltf',
            position: new this.THREE.Vector3(-6, 0, 6),
            physics: true,
          },
          {
            asset: 'Prop_Computer.gltf',
            position: new this.THREE.Vector3(0, 0, -8),
            physics: true,
          },
        )
        break
      ...
      case 'laboratory':
        props.push(
          {
            asset: 'Prop_Computer.gltf',
            position: new this.THREE.Vector3(-4, 0, -4),
            physics: true,
          },
          {
            asset: 'Prop_Chest.gltf',
            position: new this.THREE.Vector3(4, 0, -4),
            physics: true,
          },
          {
            asset: 'Prop_AccessPoint.gltf',
      ...
      case 'control_room':
        props.push(
          {
            asset: 'Prop_Computer.gltf',
            position: new this.THREE.Vector3(0, 0, -6),
            physics: true,
          },
          {
            asset: 'Prop_Light_Floor.gltf',
            position: new this.THREE.Vector3(-4, 0, 4),
            physics: false,
          },
          {
  ...
  private createFallbackRoom(
    ...
    const floorGeometry = new this.THREE.PlaneGeometry(
      config.size.width,
      config.size.length,
    )
  ...
  private createProceduralWalls(
    ...
    const wallConfigs = [
      {
        pos: [0, config.size.height / 2, -config.size.length / 2],
        size: [config.size.width, config.size.height, wallThickness],
        side: 'back',
      },
      {
        pos: [0, config.size.height / 2, config.size.length / 2],
      ...
      {
        pos: [-config.size.width / 2, config.size.height / 2, 0],
        size: [wallThickness, config.size.height, config.size.length],
        side: 'left',
      },
      {
        pos: [config.size.width / 2, config.size.height / 2, 0],
    ...
    wallConfigs.forEach(wallConfig => {
      const wallGroup = new this.THREE.Group()
      const wallGeometry = new this.THREE.BoxGeometry(
        wallConfig.size[0],
        wallConfig.size[1],
        wallConfig.size[2],
      )
  ...
  private createMinimalRoom(config: RoomConfig, level: BaseLevel): THREE.Group {
    ...
    try {
      ;(level as any).createFloorCollision(
        'main',
        {
          width: config.size.width,
          height: config.size.length,
