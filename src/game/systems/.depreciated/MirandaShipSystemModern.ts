...
export class MirandaShipSystemModern extends BaseLevelGenerator {
  ...
  constructor(dependencies: LevelGeneratorDependencies) {
    super(dependencies)
    console.log(
      '🚀 MirandaShipSystemModern created with standardized interface',
    )
  }
  ...
  private async setupSpaceshipEnvironment(): Promise<void> {
    ...
    const directionalLight = new this.dependencies.THREE.DirectionalLight(
      0x8888ff,
      0.7,
    )
    ...
    // Emergency lighting
    const emergencyLight = new this.dependencies.THREE.PointLight(
      0xff4444,
      0.5,
      20,
    )
  ...
  private async createBridgeLayout(): Promise<void> {
    ...
    const console = new this.dependencies.THREE.Mesh(
      consoleGeometry,
      consoleMaterial,
    )
  ...
  private async createControlPanels(): Promise<void> {
    ...
    for (const pos of panelPositions) {
      ...
      const panel = new this.dependencies.THREE.Mesh(
        panelGeometry,
        panelMaterial,
      )
  ...
  private updateShipAnimations(deltaTime: number): void {
    ...
    const emergencyLight = this.spaceshipLights.find(
      light =>
        light instanceof this.dependencies.THREE.PointLight &&
        light.color.r > 0.5,
    )
