...
const loadStarTexture = async (
  ...
  // Multi-layer glow
  const glowLayers = [
    { radius: finalRadius * 6.0, opacity: 0.1 + glowPhase * 0.05, blur: 30 },
    { radius: finalRadius * 4.5, opacity: 0.15 + glowPhase * 0.06, blur: 25 },
    { radius: finalRadius * 3.5, opacity: 0.2 + glowPhase * 0.08, blur: 20 },
    { radius: finalRadius * 2.5, opacity: 0.3 + glowPhase * 0.1, blur: 15 },
    { radius: finalRadius * 1.8, opacity: 0.5 + glowPhase * 0.15, blur: 10 },
    { radius: finalRadius * 1.2, opacity: 0.8 + glowPhase * 0.2, blur: 5 },
  ]
...
export class StarNavigationSystemModern extends BaseLevelGenerator {
  ...
  constructor(dependencies: LevelGeneratorDependencies) {
    ...
    this.optimizationLevel =
      optimizationManager?.getOptimizationLevel() || 'high'

    // Set up event handlers
    this.handleUIClearAll = this.clearSelection.bind(this)

    console.log(
      '⭐ StarNavigationSystemModern created with standardized interface',
    )
  }
  ...
  public async initialize(config?: any): Promise<void> {
    try {
      ...
      try {
        ...
        console.warn(
          '⭐ StarNavigationSystem: Failed to initialize texture atlas, falling back to individual textures:',
          error,
        )
      }
      ...
      this.dependencies.eventBus.on('game.state.changed', (data: any) => {
        if (data.newState && data.newState.timelineEvents) {
          ...
          if (
            newEvents.length > 0 &&
            newEvents.length !== this.timelineEvents.length
          ) {
            this.timelineEvents = newEvents
            console.log(
              `⭐ Received ${this.timelineEvents.length} timeline events from GameStateManager`,
            )
      ...
      console.log(
        '⭐ StarNavigationSystem waiting for timeline events from GameStateManager...',
      )
  ...
  private async createStars(): Promise<void> {
    ...
    console.log(
      `⭐ Created ${starEvents.length} stars in ${Object.keys(eventsByEra).length} constellations`,
    )
  }

  private async createConstellationForEra(
    era: string,
    eraEvents: any[],
  ): Promise<void> {
    ...
    const pattern =
      constellationPatterns[
        config.pattern as keyof typeof constellationPatterns
      ] || constellationPatterns.scattered
    ...
    for (let eventIndex = 0; eventIndex < eraEvents.length; eventIndex++) {
      ...
      const extraVariation =
        eventIndex >= pattern.length
          ? {
              azOffset: (Math.random() - 0.5) * 30,
              elOffset: (Math.random() - 0.5) * 20,
            }
          : { azOffset: 0, elOffset: 0 }

      // Calculate final position using spherical coordinates
      const azimuthDeg =
        config.centerAzimuth +
        patternPosition.azOffset +
        extraVariation.azOffset
      ...
      await this.createStarAtPosition(
        event,
        azimuthRad,
        polarAngleRad,
        era,
  ...
  private async createStarAtPosition(
    event: any,
    azimuthRad: number,
  ...
  private async createStar(event: any): Promise<void> {
    ...
    console.warn(
      'createStar called - this should not happen with the new architecture',
    )
  }
  ...
  private async createConstellationLines(): Promise<void> {
    ...
    try {
      Object.entries(starsByEra).forEach(([era, stars]) => {
        ...
        const config =
          constellationConfig[era as keyof typeof constellationConfig]
        if (!config) return

        const connections =
          connectionPatterns[
            config.pattern as keyof typeof connectionPatterns
          ] || connectionPatterns.scattered
        ...
        if (positions.length > 0) {
          ...
          lineGeometry.setAttribute(
            'position',
            new this.dependencies.THREE.Float32BufferAttribute(positions, 3),
          )
          ...
          const constellationLine = new this.dependencies.THREE.LineSegments(
            lineGeometry,
            lineMaterial,
          )
          ...
          console.log(
            `⭐ Created ${positions.length / 6} constellation lines for era "${era}" with color ${eraColor}`,
          )
        }
      })
    } catch (error) {
    ...
    if (totalConnections > 0) {
      console.log(
        `⭐ Total constellation lines created: ${totalConnections} connections using proper patterns and era colors`,
      )
    } else {
  ...
  private updateStarsWithTimelineData(): void {
    ...
    this.starSprites.forEach((star, starId) => {
      const eventData = this.timelineEvents.find(
        e => (e.id || e.uniqueId) === starId,
      )
      if (eventData) {
    ...
    if (this.starTextureAtlas && this.starAtlasTexture) {
      ...
      this.starSprites.forEach((star, starId) => {
        try {
          ...
          const frameIndex = this.starTextureAtlas.getFrameForTime(
            adjustedTime,
            animationSpeed,
          )
          ...
          if (
            star.material.map &&
            star.material.map.repeat &&
            star.material.map.offset
          ) {
  ...
  private updateSelectionHighlight(deltaTime: number): void {
    ...
    if (this.selectedStar) {
      const selectedSprite = this.starSprites.get(
        (this.selectedStar as any).id || (this.selectedStar as any).uniqueId,
      )
      if (selectedSprite) {
        const time = Date.now() * 0.001
   ...
   */
  private getScreenPosition(object3D: THREE.Object3D): {
    x: number
    y: number
    isInFront: boolean
  } {
