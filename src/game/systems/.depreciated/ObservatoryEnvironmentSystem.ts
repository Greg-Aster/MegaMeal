...
export class ObservatoryEnvironmentSystem extends BaseLevelGenerator {
  ...
  constructor(dependencies: LevelGeneratorDependencies) {
    super(dependencies)
    console.log(
      '🌍 ObservatoryEnvironmentSystem created with standardized interface',
    )
  }
  ...
  public getHeightAt(x: number, z: number): number {
    ...
    if (distanceFromCenter < this.terrainParams.hillRadius) {
      const heightMultiplier = Math.cos(
        (distanceFromCenter / this.terrainParams.hillRadius) * Math.PI * 0.5,
      )
      height =
        this.terrainParams.baseGroundLevel +
        this.terrainParams.hillHeight * heightMultiplier * heightMultiplier
    } else {
    ...
    if (distanceFromCenter >= this.terrainParams.islandRadius) {
      ...
      height =
        this.terrainParams.baseGroundLevel - Math.pow(voidDistance * 0.1, 2)
    }
  ...
  private calculateSpawnPoint(): void {
    ...
    console.log(
      `🎯 Calculated spawn point: [${spawnX}, ${spawnY.toFixed(2)}, ${spawnZ}]`,
    )
  }
  ...
  private setupLevelEffects(): void {
    ...
    if (renderer) {
      ...
      console.log(
        '🌟 Observatory bloom effects applied with corrected threshold:',
        correctedBloomConfig,
      )
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      textureLoader.load(
        ...
        (skyTexture: any) => {
          ...
          skyTexture.mapping =
            this.dependencies.THREE.EquirectangularReflectionMapping
          ...
          // Create skybox mesh
          const skyGeometry = new this.dependencies.THREE.SphereGeometry(
            1000,
            60,
            40,
          )
          this.skyboxMesh = new this.dependencies.THREE.Mesh(
            skyGeometry,
            skyMaterial,
          )
  ...
  private async createLighting(config?: any): Promise<void> {
    ...
    const optimizationManager =
      this.dependencies.engine.getOptimizationManager()
    const isMobile =
    ...
    const mainLightColor = new this.dependencies.THREE.Color(
      lightingConfig.mainColor,
    )
    ...
    // Create fill light
    const fillLightColor = new this.dependencies.THREE.Color(
      lightingConfig.fillColor,
    )
    ...
    const ambientLightColor = new this.dependencies.THREE.Color(
    ...
    console.log(
      '✅ Observatory lighting created with data-driven configuration:',
      {
  ...
  private async createGround(): Promise<void> {
    ...
    groundGeometry = new this.dependencies.THREE.PlaneGeometry(
      500,
      500,
      terrainSegments.width,
      terrainSegments.height,
    )
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      if (distanceFromCenter < hillRadius) {
        const heightMultiplier = Math.cos(
          (distanceFromCenter / hillRadius) * Math.PI * 0.5,
        )
        height =
          baseGroundLevel + hillHeight * heightMultiplier * heightMultiplier
      } else {
    ...
    // Create ground mesh
    this.groundMesh = new this.dependencies.THREE.Mesh(
      groundGeometry,
      groundMaterial,
    )
    if (this.groundMesh) {
  ...
  private getSurfaceNoise(x: number, z: number): number {
    ...
    for (const octave of octaves) {
      ...
      const noiseValue =
        Math.sin((x1 + octave.phaseX) * octave.freq) *
        Math.cos((z1 + octave.phaseZ) * octave.freq)
  ...
  private createIntelligentTerrainMaterial(): THREE.Material {
    try {
      ...
      console.log(
        `🎯 Terrain Material: Creating for quality level ${optimizationLevel}`,
      )
  ...
  private async createVegetation(): Promise<void> {
    ...
    console.log(
      `🌱 Creating natural vegetation (${vegetationSettings.maxInstances} max instances)...`,
    )
  ...
  private getIntelligentVegetationSettings(): {
  ...
  private async createGrassPatches(): Promise<void> {
    console.log(
      '🌱 Loading grass assets (CLUSTERED approach for performance)...',
    )
    ...
    try {
            ...
            if (child instanceof this.dependencies.THREE.Mesh) {
              ...
              if (child.material) {
                ...
                if (child.material && child.material.map) {
                  const newMaterial =
                    new this.dependencies.THREE.MeshLambertMaterial({
      ...
      console.log(
        `🌱 Creating ${grassCount} individual grass objects (will be auto-clustered by OptimizationManager)`,
      )
      ...
      console.log(
        `✅ Created ${grassCount} individual grass objects for automated clustering`,
      )
    } catch (error) {
  ...
  private async createStonesAndRocks(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < rockCount; i++) {
        ...
        const rockMesh = new this.dependencies.THREE.Mesh(
          rockGeometry,
          rockMaterial,
        )
   ...
   */
  private createLODVegetation(
    instance: THREE.Object3D,
    position: THREE.Vector3,
  ): THREE.LOD | THREE.Object3D {
    try {
      const optimizationManager =
        this.dependencies.engine.getOptimizationManager()
      const optimizationLevel = optimizationManager.getOptimizationLevel()
      ...
      if (optimizationLevel === 'high' || optimizationLevel === 'ultra') {
        const mediumDetailInstance =
          this.createReducedDetailVegetation(instance)
  ...
  private createReducedDetailVegetation(
  ...
  private createFallbackGrass(): void {
    ...
    const grassMaterial = new this.dependencies.THREE.MeshLambertMaterial({
      color: 0x4a7c59,
    })
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const grass = new this.dependencies.THREE.Mesh(
        grassGeometry,
        grassMaterial,
      )
   ...
   */
  private registerObjectForOptimization(
    ...
    try {
      ...
      if (
        optimizationManager &&
        typeof optimizationManager.registerObject === 'function'
      ) {
        ...
        console.log(
          `🎯 Marked object for optimization: ${object.name || 'unnamed'} (${type}, ${priority})`,
        )
      }
    } catch (error) {
