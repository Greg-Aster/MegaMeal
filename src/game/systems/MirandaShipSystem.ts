...
export class MirandaShipSystem implements IDisposable {
  ...
  private addVectorAccentLines(x: number, y: number, z: number, material: any): void {
    // Create geometric accent pattern
    const lines = [
      // Horizontal lines
      [new this.THREE.Vector3(x - 1.5, y + 1, z), new this.THREE.Vector3(x + 1.5, y + 1, z)],
      [new this.THREE.Vector3(x - 1.5, y, z), new this.THREE.Vector3(x + 1.5, y, z)],
      [new this.THREE.Vector3(x - 1.5, y - 1, z), new this.THREE.Vector3(x + 1.5, y - 1, z)],
      // Vertical accents
      [new this.THREE.Vector3(x - 1, y + 1.2, z), new this.THREE.Vector3(x - 1, y - 1.2, z)],
      [new this.THREE.Vector3(x + 1, y + 1.2, z), new this.THREE.Vector3(x + 1, y - 1.2, z)]
    ];
  ...
  private async createVectorViewscreen(): Promise<void> {
    ...
    const borderGeometry = new this.THREE.BufferGeometry().setFromPoints(borderPoints);
  ...
  private async createCommandStations(): Promise<void> {
    ...
    const leftWorkstation = new this.THREE.Mesh(workstationGeometry, consoleMaterial);
    ...
    const rightWorkstation = new this.THREE.Mesh(workstationGeometry, consoleMaterial);
  ...
  private async setupStoryElements(storyConfig: any): Promise<void> {
    if (!storyConfig) {
      console.log('📖 No story configuration provided, using default Miranda investigation elements...');
