...
import type { GameSettings, GameStats } from './GameState'
...
export class GameReducer {
  ...
  private static handleLevelTransitionStart(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLevelTransitionSuccess(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLevelTransitionFailure(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLevelCompleted(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleStarSelected(
    state: GameState,
    action: GameAction,
  ): GameState {
    ...
    if (!star || !star.uniqueId) {
      console.warn(
        '⚠️ Reducer: Attempted to select invalid star data. This may indicate an issue with an event listener. Star deselection should use STAR_DESELECTED action.',
        star,
      )
    ...
    if (
      selectionMethod &&
      selectionMethod in state.sessionData.selectionMethods
    ) {
      state.sessionData.selectionMethods[
        selectionMethod as keyof typeof state.sessionData.selectionMethods
      ]++
    }
  ...
  private static handleStarDeselected(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleStarDiscovered(
    state: GameState,
    action: GameAction,
  ): GameState {
    ...
    if (!state.discoveredStars.has(star.uniqueId)) {
      ...
      if (!state.sessionData.discoveryMethods) {
        state.sessionData.discoveryMethods = {
          exploration: 0,
          timeline: 0,
          interaction: 0,
        }
  ...
  private static handleStatsUpdate(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleStatsIncrement(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleStatsReset(
    state: GameState,
    action: GameAction,
  ): GameState {
    state.gameStats = {
  ...
  private static handleSettingsUpdate(
    state: GameState,
    action: GameAction,
  ): GameState {
    const { updates } = action.payload
  ...
  private static handleSettingsReset(
    state: GameState,
    action: GameAction,
  ): GameState {
    state.settings = {
  ...
  private static handleContentUnlocked(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleItemCollected(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleInteractionRecorded(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleSaveGameStart(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleSaveGameSuccess(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleSaveGameFailure(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLoadGameStart(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLoadGameSuccess(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleLoadGameFailure(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleGameInitialized(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleGamePaused(
    state: GameState,
    action: GameAction,
  ): GameState {
  ...
  private static handleGameResumed(
    state: GameState,
    action: GameAction,
  ): GameState {
    ...
    state.sessionData.totalPauseTime =
      (state.sessionData.totalPauseTime || 0) + pauseDuration
    state.sessionData.resumeTime = resumeTime

    return state
  }

  private static handleGameReset(
    state: GameState,
    action: GameAction,
  ): GameState {
    const { resetReason, preserveSettings } = action.payload

    const settingsToPreserve = preserveSettings
      ? { ...state.settings }
      : undefined
  ...
  private static handleTimelineEventsSet(
    state: GameState,
    action: GameAction,
  ): GameState {
