...
export class LevelManager {
  ...
  /**
   * Register a level class or factory w<!-- Inside your StarCard.svelte or similar component -->
   {#if $selectedStar && .isLevel}
     <button on:click={() => gameActions.transitionToLevel(.levelId)}>
       Enter Level
     </button>
   {/if}
   ith the manager
  ...
  public registerLevel(levelId: string, levelProvider: LevelProvider): void {
    ...
    const type =
      typeof levelProvider === 'function' && levelProvider.prototype
        ? 'constructor'
        : 'factory'
  ...
  /**
   * Processes the level manifest to extract timeline events for the star map.
   * This is a critical step to ensure levels appear as stars.
   * This should be called once at startup by the GameManager.
   * @param manifest The loaded level-manifest.json content.
   * @returns An array of TimelineEvent objects for the levels.
   */
  public processLevelManifest(manifest: any): TimelineEvent[] {
    if (!manifest || !manifest.levels) {
      console.error(
        '❌ Invalid level manifest provided to processLevelManifest.',
      )
      return []
    }

    const levelTimelineEvents: TimelineEvent[] = []

    for (const levelId in manifest.levels) {
      const levelData = manifest.levels[levelId]
      if (levelData && levelData.timelineEvent) {
        // Ensure the event knows it's a level for proper handling
        levelData.timelineEvent.isLevel = true
        // Add the levelId to the event data so the UI knows which level to transition to
        levelData.timelineEvent.levelId = levelId
        levelTimelineEvents.push(levelData.timelineEvent)
      }
    }

    if (levelTimelineEvents.length > 0) {
      console.log(
        `⭐ Found ${levelTimelineEvents.length} level-based timeline events to populate star map.`,
      )
    }
    return levelTimelineEvents
  }
   ...
   */
  public async transitionToLevel(
    levelId: string,
    transitionData?: any,
  ): Promise<boolean> {
    ...
    try {
      ...
      if (this.currentLevel) {
        console.log(
          `🧹 Performing cleanup for current level: ${this.currentLevel.getLevelId()}`,
        )
        ...
        console.log(
          `🧹 Disposing current level: ${this.currentLevel.getLevelId()}`,
        )
 ...
 */
export type LevelConstructor = new (
  engine: Engine,
