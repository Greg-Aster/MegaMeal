...
import type { GameSettings, GameStats, StarData } from '../state/GameState'
...
export function initializeGameStore(
  ...
  gameStateManager.subscribe(
    (action: GameAction, previousState: GameState, newState: GameState) => {
...
export const progressPercentage: Readable<number> = derived(
  ...
  ([$completedLevels, $unlockedContent]) => {
    ...
    const progress =
      ($completedLevels.size / totalLevels) * 0.7 +
...
// Save/Load stores
export const saveInfo: Readable<any> = derived(gameState, $gameState => {
...
export const gameActions = {
  ...
  selectStar: (
    star: StarData,
    method: 'click' | 'touch' | 'keyboard' = 'click',
  ) => {
    if (!eventBus) return
