...
export class StateValidator {
  ...
  private initializeDefaultRules(): void {
    ...
    this.addRule({
      ...
      validate: state => {
        ...
        if (
          state.settings.mouseSensitivity < 0.1 ||
          state.settings.mouseSensitivity > 5
        ) {
          warnings.push({
            ...
            suggestion:
              'Consider setting mouse sensitivity between 0.5 and 2.0',
          })
        }
      ...
      autoFix: state => {
        // Fix audio volume
        state.settings.audioVolume = Math.max(
          0,
          Math.min(1, state.settings.audioVolume),
        )

        // Fix mouse sensitivity
        state.settings.mouseSensitivity = Math.max(
          0.1,
          Math.min(5, state.settings.mouseSensitivity),
        )
        state.settings.touchSensitivity = Math.max(
          0.1,
          Math.min(5, state.settings.touchSensitivity),
        )
    ...
    this.addRule({
      ...
      validate: state => {
        ...
        if (
          state.selectedStar &&
          !state.discoveredStars.has(state.selectedStar.uniqueId)
        ) {
          warnings.push({
  ...
  private validateLevelTransition(
    action: GameAction,
    currentState: GameState,
  ): ValidationResult {
