...
export const GameActions = {
  // Level actions
  levelTransitionStart: (from: string, to: string, transitionData?: any): LevelTransitionStartAction => ({
    type: GameActionTypes.LEVEL_TRANSITION_START,
    payload: { from, to, transitionData },
    meta: { timestamp: Date.now(), source: 'LevelManager' }
  }),
  
  levelTransitionSuccess: (from: string, to: string, transitionTime: number): LevelTransitionSuccessAction => ({
    type: GameActionTypes.LEVEL_TRANSITION_SUCCESS,
    payload: { from, to, transitionTime },
    meta: { timestamp: Date.now(), source: 'LevelManager' }
  }),
  
  levelTransitionFailure: (from: string, to: string, error: Error): LevelTransitionFailureAction => ({
    type: GameActionTypes.LEVEL_TRANSITION_FAILURE,
    payload: { from, to, error },
    meta: { timestamp: Date.now(), source: 'LevelManager' }
  }),
  
  levelCompleted: (levelId: string, completionTime: number, stats: any): LevelCompletedAction => ({
  ...
  // Star actions
  starSelected: (star: StarData, selectionMethod: 'click' | 'touch' | 'keyboard' = 'click'): StarSelectedAction => ({
  ...
  starDiscovered: (star: StarData, discoveryMethod: 'exploration' | 'timeline' | 'interaction' = 'exploration'): StarDiscoveredAction => ({
  ...
  statsIncrement: (field: keyof GameStats, amount: number = 1): StatsIncrementAction => ({
  ...
  // Progress actions
  contentUnlocked: (contentId: string, unlockReason: string): ContentUnlockedAction => ({
    type: GameActionTypes.CONTENT_UNLOCKED,
    payload: { contentId, unlockReason },
    meta: { timestamp: Date.now(), source: 'GameStateManager' }
  }),
  
  itemCollected: (itemId: string, itemType: string, collectionMethod: string): ItemCollectedAction => ({
  ...
  interactionRecorded: (interactionType: string, objectId?: string, position?: { x: number; y: number; z: number }, data?: any): InteractionRecordedAction => ({
  ...
  // Session actions
  sessionStart: (sessionId: string, startTime: number, previousSession?: any): SessionStartAction => ({
    type: GameActionTypes.SESSION_START,
    payload: { sessionId, startTime, previousSession },
    meta: { timestamp: Date.now(), source: 'GameManager' }
  }),
  
  timeUpdate: (deltaTime: number, totalTime: number, sessionTime: number): TimeUpdateAction => ({
  ...
  saveGameSuccess: (saveTime: number, saveType: 'manual' | 'auto' | 'checkpoint', saveSize?: number): SaveGameSuccessAction => ({
    type: GameActionTypes.SAVE_GAME_SUCCESS,
    payload: { saveTime, saveType, saveSize },
    meta: { timestamp: Date.now(), source: 'GameStateManager' }
  }),
  
  saveGameFailure: (error: Error, saveType: 'manual' | 'auto' | 'checkpoint'): SaveGameFailureAction => ({
  ...
  // Error actions
  errorOccurred: (error: Error, context: ErrorContext, recoverable: boolean): ErrorOccurredAction => ({
    type: GameActionTypes.ERROR_OCCURRED,
    payload: { error, context, recoverable },
    meta: { timestamp: Date.now(), source: 'ErrorHandler' }
  }),
  
  errorRecovered: (originalError: Error, recoveryMethod: string, success: boolean): ErrorRecoveredAction => ({
  ...
  // Game lifecycle actions
  gameInitialized: (version: string, initTime: number, config: any): GameInitializedAction => ({
    type: GameActionTypes.GAME_INITIALIZED,
    payload: { version, initTime, config },
    meta: { timestamp: Date.now(), source: 'GameManager' }
  }),
  
  gamePaused: (pauseReason: 'user' | 'system' | 'error', pauseTime: number): GamePausedAction => ({
    type: GameActionTypes.GAME_PAUSED,
    payload: { pauseReason, pauseTime },
    meta: { timestamp: Date.now(), source: 'GameManager' }
  }),
  
  gameResumed: (pauseDuration: number, resumeTime: number): GameResumedAction => ({
    type: GameActionTypes.GAME_RESUMED,
    payload: { pauseDuration, resumeTime },
    meta: { timestamp: Date.now(), source: 'GameManager' }
  }),
  
  gameReset: (resetReason: 'user' | 'error' | 'system', preserveSettings?: boolean): GameResetAction => ({
 ...
 */
export function isActionType<T extends GameAction>(action: GameAction, type: string): action is T {
 ...
 */
export function createAction(type: string, payload?: any, source?: string, context?: any): GameAction {
  return {
    type,
    payload,
    meta: {
