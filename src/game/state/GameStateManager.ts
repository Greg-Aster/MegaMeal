...
export class GameStateManager {
  ...
  private internalDispatch(action: GameAction): void {
    ...
    try {
      ...
      const actionValidation = this.stateValidator.validateAction(
        processedAction,
        this.gameState,
      )
      if (!actionValidation.isValid) {
        console.warn(
          `Action validation failed for ${processedAction.type}:`,
        ...
        for (const error of actionValidation.errors) {
          if (error.severity === 'critical') {
            throw new Error(
              `Critical action validation error: ${error.message}`,
            )
          }
        }
      }
      ...
      if (!stateValidation.isValid) {
        console.warn(
          `State validation failed after ${processedAction.type}:`,
          stateValidation.errors,
        )

        // Attempt auto-fix for recoverable errors
        const hasRecoverableErrors = stateValidation.errors.some(
        ...
        if (hasRecoverableErrors) {
          ...
          if (fixValidation.isValid) {
            ...
            console.error(
              '❌ Auto-fix failed, some errors remain:',
        ...
        const criticalErrors = stateValidation.errors.filter(
          error => error.severity === 'critical' && !error.recoverable,
        )
        if (criticalErrors.length > 0) {
          throw new Error(
            `Critical state validation errors: ${criticalErrors.map(e => e.message).join(', ')}`,
          )
        }
      }
      ...
      // Track performance
      this.trackActionPerformance(
        processedAction.type,
  ...
  public async saveGame(
    ...
    try {
      ...
      if (!validation.isValid) {
        console.warn(
          '💾 Saving state with validation errors:',
          validation.errors,
        )
  ...
  public async loadGame(): Promise<boolean> {
    ...
    try {
      ...
      const migrationRequired =
        meta.version !== this.gameState.sessionData.saveVersion
      ...
      if (!validation.isValid) {
        console.warn(
          '📁 Loaded game state has validation errors:',
          validation.errors,
        )
  ...
  private setupMiddleware(): void {
    ...
    this.middleware.push({
      ...
      after: (action, _previousState, _newState) => {
        ...
        if (
          action.type.includes('TRANSITION') ||
          action.type.includes('SELECTED')
        ) {
  ...
  private performSaveMigration(fromVersion: string): void {
    console.log(
      `🔄 Migrating save data from version ${fromVersion} to ${this.gameState.sessionData.saveVersion}`,
    )
