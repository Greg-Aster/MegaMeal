...
function setupDebugGUI() {
  ...
  // State controls
  stateFolder
    .add(controls, 'logActions')
    .name('Log Actions')
    .onChange(toggleActionLogging)
  stateFolder
    .add({ validateState: () => validateCurrentState() }, 'validateState')
    .name('Validate State')
  stateFolder
    .add({ resetState: () => resetGameState() }, 'resetState')
    .name('Reset State')
  ...
  // Validator controls
  validatorFolder
    .add(controls, 'showValidationErrors')
    .name('Show Validation Errors')
  validatorFolder.add(controls, 'enableAutoFix').name('Enable Auto-Fix')
  validatorFolder
    .add({ runValidation: () => runFullValidation() }, 'runValidation')
    .name('Run Validation')
  validatorFolder
    .add({ autoFixState: () => autoFixCurrentState() }, 'autoFixState')
    .name('Auto-Fix State')
  ...
  optimizationFolder
    .add(controls, 'targetFPS', 30, 120, 1)
    .name('Target FPS')
    .onChange(setTargetFPS)
  optimizationFolder
    .add(controls, 'autoOptimize')
    .name('Auto-Optimize')
  ...
  // Time controls
  timeFolder
    .add(controls, 'timeMultiplier', 0.1, 5.0, 0.1)
    .name('Time Multiplier')
    .onChange(setTimeMultiplier)
  timeFolder
    .add(controls, 'pauseTime')
    .name('Pause Time')
    .onChange(toggleTimeTracking)

  // === GRAPHICS CONTROLS ===
  const graphicsFolder = gui.addFolder('🎨 Graphics')
  graphicsFolder
    .add(controls, 'vectorMode')
    .name('Vector Mode')
    .onChange(toggleVectorMode)
  graphicsFolder
    .add(controls, 'bloom', 0, 2, 0.1)
    .name('Bloom')
    .onChange(setBloom)
  graphicsFolder
    .add(controls, 'outlines')
    .name('Outlines')
  ...
  levelFolder
    .add({ observatory: () => transitionToLevel('observatory') }, 'observatory')
    .name('Observatory')
  levelFolder
    .add({ miranda: () => transitionToLevel('miranda') }, 'miranda')
    .name('Miranda')
  levelFolder
    .add({ restaurant: () => transitionToLevel('restaurant') }, 'restaurant')
    .name('Restaurant')
  levelFolder
    .add({ resetCamera: () => resetCameraView() }, 'resetCamera')
    .name('Reset Camera')
  ...
  saveFolder
    .add({ clearSave: () => clearSaveData() }, 'clearSave')
    .name('Clear Save')
...
function updatePerformanceMetrics() {
  ...
  if (performance && (performance as any).memory) {
    performanceMetrics.memory = Math.round(
...
function updateGameStateMetrics() {
  ...
  gameStateMetrics.averageActionTime = Number(
    metrics.averageActionTime.toFixed(2),
  )
  ...
  gameStateMetrics.validationSuccessRate = Number(
...
function updateOptimizationMetrics() {
  ...
  optimizationMetrics.currentFPS = Math.round(
    1000 / (engine.getTime().deltaTime || 16),
  )
...
function updateTimeMetrics() {
  ...
  timeMetrics.totalPlayTime = Number(
    (currentTime.totalPlayTime / 1000).toFixed(1),
  )
  timeMetrics.sessionTime = Number(
    (currentTime.totalPlayTime / 1000).toFixed(1),
  ) // Same as total for now
  timeMetrics.timeExplored = Number(
    (currentTime.timeExplored / 1000).toFixed(1),
  )
